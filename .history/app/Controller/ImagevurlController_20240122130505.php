...
public function downloadImages() {
    ...
    if ($this->request->is('post')) {
        $clientId = $this->request->getData('Uploadimglink.client_id');
        $skuCodesInput = $this->request->getData('Uploadimglink.sku_codes');

        // Validate SKU codes input
        $validator = $this->Uploadimglink->validator();
        $validator
            ->requirePresence('sku_codes', 'create', 'SKU Codes are required')
            ->notEmptyString('sku_codes', 'SKU Codes cannot be empty');
        ...
        if (empty($errors)) {
            ...
            // Set validation errors to be displayed in the view
