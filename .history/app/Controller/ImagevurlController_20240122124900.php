...
public function index(){
    ...
    if ($this->request->is('post')) {
        $clientId = $this->request->data['Uploadimglink']['client_ref_id'];
        ...
        $conditions = ['Uploadimglink.client_ref_id' => $clientId, 'Uploadimglink.SKU_CODE IN' => $skuCodes];
...
public function downloadImages() {
    ...
    if ($this->request->is('post')) {
        ...
        // Validate SKU codes input
        if (empty($skuCodesInput)) {
            $this->Flash->error('Please enter SKU codes.');
            return $this->redirect(['action' => 'downloadImages']);
        }
        ...
        $conditions = ['Uploadimglink.client_ref_id' => $clientId];

        // Add condition to filter by SKU codes if provided
        if (!empty($skuCodes)) {
            $conditions['Uploadimglink.SKU_CODE IN'] = $skuCodes;
        } else {
            $this->Flash->error('Please enter at least one SKU code.');
            return $this->redirect(['action' => 'downloadImages']);
        }
        ...
        if (empty($images)) {
            $this->Flash->error('No images found for the provided SKU codes.');
            return $this->redirect(['action' => 'downloadImages']);
        }
        ...
        // Output CSV header
        fputcsv($output, ['client id', 'SKU_CODE', 'Image 1', 'Image 2', 'Image 3', 'Image 4', 'Image 5', 'Image 6', 'Image 7', 'Image 8']);
