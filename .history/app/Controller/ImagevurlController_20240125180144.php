...
public function clientImages() {
    ...
    if ($this->request->is('post')) {
        ...
        $this->Paginator->settings = array(
            'limit' => 10, // Adjust the number of images per page as needed
            'conditions' => array('Uploadimglink.client_ref_id' => $this->request->data('Uploadimglink.client_id')),
        );

        // Get paginated data
        $images = $this->Paginator->paginate('Uploadimglink');
    ...
    if ($this->request->is('post')) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            if (!empty($csvData)) {
                ...
                $this->Session->setFlash('The csv file is empty ');
            }
        } else {
...
//if images are stored in local uploads folder ... unlink process 
