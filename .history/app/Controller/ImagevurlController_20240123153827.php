...
public $helpers = array('Html', 'Form', HttpSocket', 'Network/Http'));
                ...
                foreach ($images as $image) {
                    ...
                    for ($i = 1; $i <= 5; $i++) {
                        ...
                        if (isset($image['Uploadimglink'][$imageLinkIndex])) {
                            ...
                            if (!empty($imageLink)) {
                                // Download the image content from the link using HTTP socket
                                $httpSocket = new HttpSocket();
                                $fileContent = $httpSocket->get($imageLink)->body;
                                // Add file content to the zip file
                                fwrite($zipFile, "{$image['Uploadimglink']['SKU_CODE']}_image{$i}.jpg" . "\n");
                                fwrite($zipFile, $fileContent . "\n");
                            }
                        }
                    }
                }
