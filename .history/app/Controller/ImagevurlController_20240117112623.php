...
class ImagevurlController extends AppController {
    ...
    public $uses = array('Uploadimglink'); // Use the correct model name
        ...
        if ($this->request->is('post')) {
            ...
            if ($file['error'] === UPLOAD_ERR_OK) {
                ...
                if (!empty($csvData)) {
                    ...
                    $this->Session->setFlash('CSV file processed and data insertesuccessfully.');
                } else {
    ...
    private function _processCSVData($csvData) {
        foreach ($csvData as $row) {
            ...
            $data = array(
                'Uploadimglink' => array( // Use the correct model name
                    'SKU_CODE' => $skuCode,
                    'image1' => $row[1],
                    'image2' => $row[2],
                    'image3' => $row[3],
            ...
            $this->Uploadimglink->create(); // Use the correct model name
