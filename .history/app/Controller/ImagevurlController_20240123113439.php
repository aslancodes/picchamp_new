...
public function index(){
    ...
    public function downloadImagesBySku() {
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($images)) {
                ...
                $tempDirectoryName = 'images_' . time();
                $tempDirectoryPath = TMP . $tempDirectoryName;
    
                // Create a temporary directory
                mkdir($tempDirectoryPath);
    
                foreach ($images as $image) {
                    // Add each image link to the temporary directory
                    for ($i = 1; $i <= 5; $i++) {
                        $imageLinkIndex = 'image' . $i;
                        if (isset($image['Uploadimglink'][$imageLinkIndex])) {
                            $imageLink = $image['Uploadimglink'][$imageLinkIndex];
                            if (!empty($imageLink)) {
                                // Download the image content from the link
                                $fileContent = file_get_contents($imageLink);
    
                                // Save the image to the temporary directory
                                $fileName = "{$image['Uploadimglink']['SKU_CODE']}_image{$i}.jpg";
                                $filePath = $tempDirectoryPath . DS . $fileName;
                ...
                $this->createZip($tempDirectoryPath, $zipFilePath);
                ...
                $this->removeDirectory($tempDirectoryPath);
                unlink($zipFilePath);
    ...
    private function addFileToZip($zip, $filePath, $zipFileEntry) {
        $zipFile = fopen($filePath, 'rb');
        $zipFileSize = filesize($filePath);
        
        // Add file entry to the zip archive
        zip_entry_open($zip, $zipFileEntry);
        zip_entry_write($zip, fread($zipFile, $zipFileSize));
        zip_entry_close($zip);
